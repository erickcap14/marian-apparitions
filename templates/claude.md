...
If `tasks.md` exists in the project root, I **must** read it before picking up any work. It is the canonical shared work queue for all agents and the developer.

### Before starting any task

1. Read `tasks.md` in full.
2. Find tasks where `Status = [ ]` AND every ID in `Blocked By` is either `—` or has `Status = [x]`.
3. Claim the task by updating its `Status` cell to `[~]` before writing any code.

### After completing a task

1. Update the task `Status` to `[x]`.
2. Update the `Summary` table counts at the bottom of `tasks.md`.
3. If new work was discovered during the task, add it with the next available `T00N` ID.

### Never

- Start a task whose `Blocked By` dependencies are not yet `[x]`.
- Pick up a task already marked `[~]` (another agent may be working it).
- Renumber or reuse existing task IDs.
